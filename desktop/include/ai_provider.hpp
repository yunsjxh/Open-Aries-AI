...
// 原生函数调用 (function calling) 结果
struct ToolCallInfo {
    std::string id;
    std::string name;       // function name: RUN_PS, READ_FILE, etc.
...
public:
    ...
    // 发送原始 JSON 流请求（用于自定义多模态等场景）
    virtual std::pair<bool, std::string> sendRawStreamRequest(
        const std::string& jsonBody,
        StreamDeltaCallback onDelta) {
        return {false, "Not supported"};
    }

    // 流式发送消息（带 tools 原生函数调用）
    virtual std::pair<bool, std::string> sendMessageStreamWithTools(
        const std::vector<ChatMessage>& messages,
        StreamDeltaCallback onDelta,
        const std::string& systemPrompt,
        const std::string& toolsJson,
        std::vector<ToolCallInfo>& outToolCalls) {
        outToolCalls.clear();
        return sendMessageStream(messages, onDelta, systemPrompt);
    }

    // 发送原始 JSON 流请求（带 tools）
    virtual std::pair<bool, std::string> sendRawStreamRequestWithTools(
        const std::string& jsonBody,
        StreamDeltaCallback onDelta,
        std::vector<ToolCallInfo>& outToolCalls) {
        outToolCalls.clear();
        return sendRawStreamRequest(jsonBody, onDelta);
    }

    // 发送原始 JSON 非流式请求（用于多模态/视觉）
    virtual std::pair<bool, std::string> sendRawRequest(
        const std::string& jsonBody) {
        return {false, "Not supported"};
    }
